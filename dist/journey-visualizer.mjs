...
export function getCommandJourney(cmd) {
    const steps = [
...
function getCommandStep(command) {
    const commandMap = {
...
export function formatJourneyCompact(journey) {
    return journey.map(s => s.icon).join(' → ');
}
export function formatJourneyDetailed(journey) {
    return journey.map((s, i) => `${i + 1}. ${s.icon} ${s.label}`).join('\n');
}
export function getExecutionTree(journey) {
    let tree = '';
...
export class CommandHistoryManager {
    ...
    addEntry(command, journey, success) {
        this.history.push({
            command,
            journey,
            timestamp: Date.now(),
    ...
    predictNextCommand(currentCommand) {
        const suggestions = [];
