...
export class SyscallStreamer extends EventEmitter {
    ...
    calculateJourneyProgress(syscallName, totalSyscalls) {
        ...
        if (syscallName === 'execve')
            progress = Math.max(progress, 20); // Command execution started
        if (syscallName === 'open' && progress < 40)
            progress = 40; // Filesystem access
        if (syscallName === 'socket' || syscallName === 'connect')
            progress = Math.max(progress, 60); // Network operations
        if (syscallName === 'wait4' || syscallName === 'exit_group')
            progress = 100; // Command completion
        return Math.min(progress, 100);
    }
    getEstimatedStreamingTrace() {
        const executionTime = Date.now() - this.startTime;
